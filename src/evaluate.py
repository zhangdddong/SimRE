...
# reset args
args.eval_model_path = args.model_dir + '/' + 'model_last.mdl'
...
def eval_single_direction(predictor: BertPredictor,
    ...
    # entity_tensor
    # entity_tensor_numpy = entity_tensor.cpu().detach().numpy()
    # np.save('FB15k237-entity.npy', entity_tensor_numpy)
    # print(len(entity_tensor_numpy))

    hr_tensor, _ = predictor.predict_by_examples(examples)

    # vector json -> show
    # hr_tensor_numpy = hr_tensor.cpu().detach().numpy()
    # save_vector = []
    # assert len(hr_tensor_numpy) == len(examples), 'vector len error'
    # for i, example in enumerate(examples):
    #     save_vector.append({
    #         'head_vector': ','.join([str(ele) for ele in hr_tensor_numpy[i].tolist()]),
    #         'tail_id': example.tail_id
    #     })
    # json.dump(save_vector, open('./FB15k237-hr-no-rule.json', 'w', encoding='UTF-8'), ensure_ascii=False, indent=4)
    # print(len(save_vector))
